...
function moverPacman() {
    ...
    if (movimentoAtual && gameState === 'playing' && podeMoverPacman) {
        ...
        if (movimentoAtual == 39) { // Seta para a direita
            novaColuna++;
        } else if (movimentoAtual == 37) { // Seta para a esquerda
            novaColuna--;
        } else if (movimentoAtual == 38) { // Seta para cima
            novaLinha--;
        } else if (movimentoAtual == 40) { // Seta para baixo
            novaLinha++;
        }
...
window.addEventListener("keydown", function (event) {
    ...
    if ([37, 38, 39, 40].includes(tecla)) {
        ...
        } else if (tecla === 37) { // Seta para a esquerda
