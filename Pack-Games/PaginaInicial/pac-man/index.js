...
function desenharVidas() {
    ctx.font = '24px Arial';
    ctx.fillStyle = 'white';
    ctx.fillText('Vidas: ' + vidas, 900, 50); 
}

let fantasmasMovendo = false;
...
class Pacman {
    ...
    draw() {
        ctx.drawImage(this.pacmanImages[this.currentImageIndex], this.x, this.y, 40, 40);
    }
...
window.addEventListener("keydown", function(event) {
    ...
    if (map[novaLinha][novaColuna] != '-' && map[novaLinha][novaColuna] != '0') {
        ...
        // Começa a mover os fantasmas após o Pacman se mover pela primeira vez
...
function desenharPontuacao() {
    ctx.fillStyle = 'white';
    ctx.font = '20px Arial';
    ctx.fillText('Pontuação: ' + pontuacao, 900, 100); 
}
...
function animate() {
    ...
    if (fantasmasMovendo) {
        ...
        desenharFantasmas();  // Desenha os fantasmas em suas posições iniciais
    }
    ...
    map.forEach((row, linha) => {
        row.forEach((symbol, coluna) => {
            ...
            if (symbol === '-') {
                const limite = new Limite({ position });
                limites.push(limite);
            } else if (symbol != '-' && symbol != '0' && symbol != '4' ) {
